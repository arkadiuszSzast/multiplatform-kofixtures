...
//    // ---------------------------------------------------------------------------
    ...
    "tags" - {
        ...
        "primary (no tag) acts as fallback when tag is not found" {
            val registry = buildRegistry {
...
//    // ---------------------------------------------------------------------------
    ...
    "nullable derivation" - {
        ...
        "no tag and not nullable acts as fallback when tag is not found" {
            val registry = buildRegistry {
...
//    // ---------------------------------------------------------------------------
    ...
    "collectionConfig" - {
        ...
        "each type can have a different range" {
            val registry = buildRegistry {
                collections {
                    listSize = 1..1
                    setSize = 2..2
            ...
            registry.generator<List<String>>().next(random).size shouldBe 1
            registry.generator<Set<String>>().next(random).size shouldBe 2
            registry.generator<Map<String, Int>>().next(random).size shouldBe 3
        }
    }
...
//    // ---------------------------------------------------------------------------
    ...
    "factoryScope" - {
        ...
        "can access other generators with tag during registration" {
            val registry = buildRegistry {
        ...
        "when generator with tag is not found, falls back to primary" {
            val registry = buildRegistry {
                register<Int> { gen { 1 } }
                register<String> {
                    val intGenerator: Generator<Int> = registry.generator("one")
